...
def create(request):
    ...
    Also returns post statistics and recent posts to the template for display.
    """
    user = request.user
    
    # Fetch the user's profile, log and handle errors if UserProfile does not exist
    try:
...
def update(request, pk):
    ...
    """
    # Get the post by primary key or return 404 if not found
    ...
    if request.method == 'POST':
        ...
        # Redirect back to the user's profile after update
...
def delete(request, pk):
    ...
    """
    # Retrieve the post or raise a 404 if not found
    post = get_object_or_404(Post, pk=pk)
